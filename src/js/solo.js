...
if (userDataJSON) {
  userData.id = JSON.parse(userDataJSON).id;
  userData.username = JSON.parse(userDataJSON).username;
...
socket.addEventListener("message", function (event) {
  ...
  if (data.action === "roomCreated") {
    ...
    opponentData.id = data.userData.id;
    opponentData.username = data.userData.username;
    checkOpponent = true;
...
function updateStatus() {
  ...
  if (game.game_over()) {
    ...
    if (checkOpponent) saveGameResult();
  }
}

function saveGameResult() {
  // Tạo đối tượng chứa dữ liệu kết quả ván đấu cần gửi lên máy chủ
  const gameResultData = {
    ...
    result: userData.id + game.turn() === "r" ? "Loss" : "Win",
    // Thêm thông tin khác nếu cần
  };

  // Gửi dữ liệu kết quả ván đấu lên máy chủ bằng fetch hoặc axios
  fetch("http://localhost:5000/api/addNewGame", {
    method: "POST",
    headers: {
...
async function addNewGame() {
  await fetch("http://localhost:5000/api/addNewGame", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
